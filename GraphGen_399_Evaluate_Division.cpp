...
public:
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        //the core idea is start/end = start/first * first/ end
    ...
    double dfs(string src, string dst, unordered_set<string>& visited){
        //base case, and to avoid case "x/x = 1", but x is not in grapg-->should hence be -1
        ...
        return -1;//the path to dst is not found in the graph
    }
};